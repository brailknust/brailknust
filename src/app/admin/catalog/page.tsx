    ...
    <AppShell title="Programme course catalog" eyebrow="Administration">
      ...
      <div className="mt-6 grid gap-6">
        {knustCurricula.map((template) => (
          <section key={`${template.program}-${template.level}-${template.semester}`} className="rounded-lg border border-border bg-background p-5">
            <div>
            ...
            <div className="mt-4 grid gap-2">
              {template.courses.map((course) => {
                ...
                return (
                  <div key={course.code} className={`flex items-center justify-between gap-4 rounded-md border border-border p-3 ${removal ? "bg-surface opacity-70" : "bg-background"}`}>
                    <div>
                    ...
                    {removal ? (
                      <form action={restoreProgrammeCourse}>
                        {fields}
                        <button className="inline-flex h-9 items-center gap-2 rounded-md border border-border px-3 text-xs font-semibold">
        ...
        {!knustCurricula.length ? <p className="rounded-lg border border-border p-5 text-sm text-muted">No programme curriculum templates are configured.</p> : null}
      </div>
      ...
      <section className="mt-6 rounded-lg border border-border bg-background p-5">
        ...
        <div className="mt-4 grid gap-2">
          {unassignedCourses.length ? unassignedCourses.map((course) => {
            ...
            return (
              <div key={course.id} className="flex items-center justify-between gap-4 rounded-md border border-border bg-surface p-3">
                <div>
                ...
                {!hasHistory ? (
                  ...
                  <span className="rounded-full border border-border bg-background px-2.5 py-1 text-[11px] font-semibold text-muted">
                    In use
                  </span>
                )}
              </div>
