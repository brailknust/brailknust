  ...
  return (
    <AppShell title="Academic semesters" eyebrow="Academics">
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[0.7fr_1.3fr]">
        <form action={createSemester} className="rounded-lg border border-border bg-background p-5">
          ...
          <div className="mt-5 grid gap-4">
            {availableSlots.length ? (
              <select name="slot" required defaultValue={availableSlots[0] ? `${availableSlots[0].level}|${availableSlots[0].name}` : ""} className="h-11 rounded-md border border-border bg-background px-3 text-sm">
              ...
              <p className="rounded-md border border-border bg-surface p-4 text-sm text-muted">All available semester slots have been created.</p>
            )}
            <select
              name="academicYear"
              required
            ...
            <button disabled={!availableSlots.length} className="h-11 rounded-md bg-foreground px-4 text-sm font-semibold text-background disabled:cursor-not-allowed disabled:opacity-50">
              Save semester
            </button>
          </div>
        </form>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {semesters.length ? (
            semesters.map((semester) => (
              <article
                key={semester.id}
                className="rounded-lg border border-border bg-background p-5 transition hover:border-foreground"
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  {semester.isActiveForUser ? (
                    <span className="rounded-md bg-foreground px-3 py-2 text-xs font-semibold text-background">
                      Active
                    </span>
                  ) : null}
                </div>
                ...
                <div className="mt-5 flex flex-col gap-3 sm:flex-row">
                  ...
                  <form action={deleteSemester}>
                    ...
                    <ConfirmSubmitButton
                      ...
                      className="inline-flex h-10 w-full items-center justify-center rounded-md border border-border px-4 text-sm font-semibold text-muted transition hover:border-foreground hover:text-foreground"
                    >
                      Delete
                    </ConfirmSubmitButton>
                  </form>
