-- Place every provisioned level in the academic year relative to each student's active semester.
-- Custom and legacy semesters are deliberately left unchanged.
WITH active_semesters AS (
  ...
  WHERE active_semester.academic_year ~ '^\\d{4}/\\d{4}$'
